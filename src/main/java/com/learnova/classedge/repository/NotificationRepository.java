...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query("SELECT n FROM Notification n WHERE n.member = :email AND n.regDate >= :since ORDER BY n.regDate DESC")
    List<Notification> findRecentNotification(@Param("email") String email, @Param("since") LocalDateTime since);

    @Query("SELECT COUNT(n) FROM Notification n WHERE n.member = :email AND n.isRead =false AND n.regDate >= :since")
